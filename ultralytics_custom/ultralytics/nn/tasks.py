...
from ultralytics.nn.modules import (
    ...
    v10Detect,
    ConvSE, # Own Layer
    Detect_HailoFriendly, # Own layer
)
...
def parse_model(d, ch, verbose=True):
    ...
    # Белый список модулей, которые можно безопасно заменять
    safe_replacements = {
        ...
        'YOLOEDetect': YOLOEDetect,
        'YOLOESegment': YOLOESegment,
        'v10Detect': v10Detect,
        'ImagePoolingAttn': ImagePoolingAttn,
        'RepVGGDW': RepVGGDW,
        'Detect_HailoFriendly': Detect_HailoFriendly,
    }
    ...
    base_modules = frozenset(
        {
            ...
            C2fCIB,
            A2C2f,
            ConvSE,  # Добавляем ваш кастомный слой
        }
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
            ...
            if isinstance(a, str):
                with contextlib.suppress(ValueError):
                    try:
                        ...
                        if a in safe_replacements:
                            args[j] = safe_replacements[a]
            elif isinstance(a, type):
                args[j] = 1
            elif not isinstance(a, (int, float, bool, list, tuple)):
                args[j] = 1
        ...
        if m in base_modules:
            ...
            if m in repeat_modules:
                if len(args) > 0 and isinstance(args[-1], str):
                    block_str = args[-1]
                    if block_str in safe_replacements:
                        args[-1] = safe_replacements[block_str]
                    else:
                        raise ValueError(f"Custom block '{block_str}' not found in safe_replacements")
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect, Detect_HailoFriendly}
        ):
            if len(args) > 0:
            ...
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, Detect_HailoFriendly}:
                m.legacy = legacy
        ...
        print(f"DEBUG: Creating {m.__name__ if hasattr(m, '__name__') else str(m)} with args: {args}")
